...
from chaoslab.physics import DoublePendulumParams, simulate, divergence, estimate_lyapunov_slope, energy
...
def main():
    ...
    _, _, total_energy = energy(y, params)
    max_relative_energy_drift = np.max(
        np.abs((total_energy - total_energy[0]) / max(abs(total_energy[0]), 1e-12))
    )
    ...
    (data / "summary_metrics.txt").write_text(
        ...
        f"max_relative_energy_drift = {max_relative_energy_drift:.6g}\n",
        encoding="utf-8",
    )
